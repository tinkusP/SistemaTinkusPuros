...
import { asegurarIndiceAsignacionActiva, esIndiceAsignacionActivaCorrecto, esIndiceCompuestoHistoricoIncompatible, esIndiceFraternoBloque, esIndicePosicionFisica, FILTRO_ASIGNACION_ACTIVA } from "../services/AsignacionBloqueService";
...
async function ejecutar() {
  ...
  const indicesIncompatibles = indices.filter((indice) => esIndicePosicionFisica(indice) || esIndiceCompuestoHistoricoIncompatible(indice) || (esIndiceFraternoBloque(indice) && !esIndiceAsignacionActivaCorrecto(indice)));
