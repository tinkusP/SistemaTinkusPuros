...
r.put("/configuracion", authenticate, soloAdministracion, body("gestionId").isMongoId(), body("cupoMaximoHombres").isInt({ min: 0 }).toInt(), body("cupoMaximoMujeres").isInt({ min: 0 }).toInt(), body("tarifaInterno").isFloat({ min: .01 }).toFloat(), body("tarifaExterno").isFloat({ min: .01 }).toFloat(), body("vigenciaTokenHoras").isInt({ min: 1, max: 8760 }).toInt(), body("plazoPrimeraCuotaHoras").isInt({ min: 1, max: 8760 }).toInt(), body("cantidadBloques").isInt({ min: 1, max: 100 }).toInt(), handleInputErrors, guardarConfiguracionTokens);
