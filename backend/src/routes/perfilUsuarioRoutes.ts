...
router.post("/:id/autorizaciones-edicion", authenticate, soloAdministracion, param("id").isMongoId(), body("motivo").trim().notEmpty().isLength({ max: 500 }), body("campos").isArray({ min: 1 }), body("campos.*").isIn(["DATOS_PERSONALES", "CORREO_ELECTRONICO", "ORIGEN_ACADEMICO", "FOTO_PERFIL", "CARNET_ANVERSO", "CARNET_REVERSO", "REGISTRO_UNIVERSITARIO"]), handleInputErrors, autorizarEdicion);
