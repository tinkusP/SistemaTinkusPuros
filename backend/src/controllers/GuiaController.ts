...
import { LIMITES_BLOQUE, mensajeCupoCompleto, normalizarGeneroBloque, validarCupoGuia, validarCupoIntegrante } from "../services/BloqueService";
...
async function crearBloqueAdministrativo(req: Request, res: Response) {
  const nombre=String(req.body.nombre??"").trim(); if(!nombre)return res.status(400).json({error:"El nombre del bloque es obligatorio"});
...
async function renombrar(req:Request,res:Response,bloque:any){const nombre=String(req.body.nombre??"").trim();if(!nombre)return res.status(400).json({error:"El nombre del bloque es obligatorio"});const anterior=bloque.nombre;bloque.nombre=nombre;try{await bloque.save();}catch(error){return res.status(409).json({error:(error as any)?.code===11000?"Ya existe un bloque con ese nombre":"No se pudo cambiar el nombre"});}await registrarAuditoria(req,{accion:"RENOMBRAR_BLOQUE",modulo:"BLOQUES",entidad:"Bloque",entidadId:bloque._id,descripcion:`Se cambió el nombre de ${anterior} a ${bloque.nombre}`,datosAntes:{nombre:anterior},datosDespues:{nombre:bloque.nombre}});return res.json({message:"Nombre del bloque actualizado",bloque});}
