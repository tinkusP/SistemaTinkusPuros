    ...
    >
      <div className="flex h-[100dvh] min-h-0 w-full max-w-6xl flex-col overflow-hidden bg-white shadow-2xl sm:h-[94dvh] sm:rounded-3xl">
        ...
        <div className="min-h-0 flex-1 touch-pan-y overflow-x-hidden overflow-y-auto overscroll-contain p-3 [scrollbar-gutter:stable] sm:p-6">
              ...
              <section className="space-y-5">
                ...
                <Bloque titulo="Estado de la cuenta" icono={<ShieldCheck />}>
                  ...
                  {preregistroVigente && <div className="mt-4 rounded-xl border border-[#dfd2bf] bg-white p-4">
                    ...
                    <button type="button" onClick={() => setDecisionPreregistroVisible(true)} className="mt-3 w-full rounded-lg border border-[#841534] bg-white px-3 py-2 text-xs font-black text-[#841534] transition hover:bg-[#841534] hover:text-white">Editar preregistro</button>
          ...
          {decisionPreregistroVisible && <AsignarEstadoPreregistroModal usuarioId={perfil._id} nombre={nombreCompleto(perfil)} telefono={perfil.telefono} estadoInicial={preregistroVigente?.estado} observacionInicial={preregistroVigente?.observacion} modoEdicion={Boolean(preregistroVigente)} cerrar={() => setDecisionPreregistroVisible(false)} completado={(estado) => { setPreregistroAprobado(estado === "APROBADO"); void Promise.all([queryClient.invalidateQueries({ queryKey: ["preregistro-perfil", perfil._id] }), queryClient.invalidateQueries({ queryKey: ["preregistros"] })]); }} />}
