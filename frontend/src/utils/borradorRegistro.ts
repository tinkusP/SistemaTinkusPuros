...
export const limpiarBorradorRegistro = async () => { localStorage.removeItem(CLAVE_DATOS); const bd = await abrirBd(); await new Promise<void>((resolve) => { const tx = bd.transaction(ALMACEN, "readwrite"); tx.objectStore(ALMACEN).clear(); tx.oncomplete = () => resolve(); tx.onerror = () => resolve(); }); bd.close(); };
