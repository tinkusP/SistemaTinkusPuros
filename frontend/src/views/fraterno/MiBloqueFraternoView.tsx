...
function Grupo({titulo,guias}:{titulo:string;guias:any[]}){return <div><h3 className="font-black text-slate-700">{titulo}</h3><div className="mt-3 grid gap-3 sm:grid-cols-2">{guias.map(g=>{const u=g.usuarioId,ruta=u?.fotoPerfil?(u.fotoPerfil.startsWith("http")?u.fotoPerfil:`${API}${u.fotoPerfil}`):"",telefono=String(u?.telefono??"").replace(/\D/g,""),whatsapp=telefono.length>=7?`https://wa.me/${telefono.startsWith("591")?telefono:`591${telefono}`}`:"";return <article key={g._id} className="rounded-2xl border bg-slate-50 p-4 text-center">{ruta?<img src={ruta} alt={`Foto de ${nombre(g)}`} className="mx-auto h-28 w-28 rounded-2xl object-cover"/>:<div className="mx-auto grid h-28 w-28 place-items-center rounded-2xl bg-slate-200 text-3xl font-black text-[#841534]">{nombre(g).charAt(0)}</div>}<p className="mt-3 font-black">{nombre(g)}</p><p className="mt-1 text-sm text-slate-600">{u?.telefono||"Celular no registrado"}</p>{whatsapp?<a href={whatsapp} target="_blank" rel="noreferrer" className="mt-3 inline-block rounded-xl bg-emerald-600 px-4 py-2 text-sm font-black text-white">Escribir por WhatsApp</a>:null}</article>})}{!guias.length&&<p className="rounded-xl bg-slate-50 p-5 text-sm text-slate-500">Administración aún no asignó guías en este grupo.</p>}</div></div>}
