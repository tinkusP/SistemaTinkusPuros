...
export default function MisPagosView() {
  ...
  return <main className="min-h-screen bg-[#eee8dc] p-4 sm:p-8"><div className="mx-auto max-w-5xl space-y-6">
    ...
    {numeroCuotas ? <section className="rounded-2xl bg-white p-5 shadow-sm"><h2 className="text-xl font-black text-[#74122A]">Plan confirmado: {numeroCuotas} cuota{numeroCuotas > 1 ? "s" : ""}</h2><p className="mt-1 text-slate-600">Tu elección quedó guardada y ya no puede cambiarse. Corresponde la cuota {numeroPagoActual} de {numeroCuotas}; si necesitas corregir el plan, comunícate con administración.</p></section> : <PlanCuotas total={cuota.montoTotal} seleccionado={numeroCuotas} bloqueado={pagoPendiente} guardando={plan.isPending} elegir={(cantidad) => plan.mutate(cantidad)}/>}
