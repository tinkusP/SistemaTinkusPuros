          ...
          <section className="relative w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="space-y-5 p-6">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                ].map(([codigo,titulo,estado]) => { const esCarnet=codigo==="CARNET_ANVERSO"; const seleccionado=esCarnet?camposPermiso.includes("CARNET_ANVERSO")&&camposPermiso.includes("CARNET_REVERSO"):camposPermiso.includes(codigo); return <label key={codigo} className={`cursor-pointer rounded-2xl border p-4 ${seleccionado ? "border-[#841534] bg-[#841534]/5" : "border-slate-200"}`}><input type="checkbox" className="mr-3" checked={seleccionado} onChange={()=>setCamposPermiso(actual=>esCarnet?(seleccionado?actual.filter(x=>!["CARNET_ANVERSO","CARNET_REVERSO"].includes(x)):Array.from(new Set([...actual,"CARNET_ANVERSO","CARNET_REVERSO"]))):(actual.includes(codigo)?actual.filter(x=>x!==codigo):[...actual,codigo]))}/><strong>{titulo}</strong><span className="mt-1 block pl-6 text-xs text-slate-500">{estado}</span></label>;})}
              </div>
              <p className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900"><strong>Carnet completo:</strong> si el usuario elige PDF no deberá subir reverso. Si elige una imagen, el sistema le pedirá anverso y reverso.</p>
