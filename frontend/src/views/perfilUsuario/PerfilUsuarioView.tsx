              ...
              <tbody>
                {perfilesPagina.map(
                  (
                    ...
                    const puedeRecordarPago = perfil.estado === "ACTIVO" && preregistro?.estado === "APROBADO" && (!cuota || cuota.montoPagado <= 0);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4"><div className="flex flex-col items-start gap-2">{cuota ? <button type="button" onClick={() => setCuotaSeleccionada(cuota)} className="rounded-xl bg-blue-100 px-4 py-2 text-xs font-black text-blue-800">Ver pagos<span className="mt-1 block font-medium">Bs {cuota.montoPagado.toFixed(2)} / {cuota.montoTotal.toFixed(2)}</span><span className="mt-1 block">Tarifa {cuota.tipoOrigenTarifa ?? "sin clasificar"}</span></button> : preregistro ? <button type="button" disabled={vinculandoCuota} onClick={() => { const aviso = preregistro.estado === "APROBADO" ? "La cuota quedará habilitada para el usuario." : `El preregistro está ${preregistro.estado}; la cuota quedará preparada y se habilitará cuando sea aprobado.`; if (window.confirm(`Se asignará automáticamente la tarifa según el origen ${perfil.tipoOrigen.replaceAll("_", " ")}. ${aviso} ¿Continuar?`)) vincularCuota.mutate({ preregistroId: preregistro._id }); }} className="rounded-xl bg-amber-100 px-4 py-2 text-xs font-black text-amber-900 disabled:opacity-50">{vinculandoCuota ? "Asignando..." : `Asignar cuota ${normalizarOrigenAcademico(perfil.tipoOrigen).startsWith("EXTERNO") ? "externa" : "interna"}`}</button> : <span className="text-xs text-slate-400">Primero crea el preregistro</span>}{puedeRecordarPago ? (telefonoRecordatorio ? <a href={`https://wa.me/${telefonoRecordatorio}?text=${encodeURIComponent(mensajeRecordatorioPago([perfil.nombres, perfil.apellidoPaterno, perfil.apellidoMaterno].filter(Boolean).join(" ")))}`} target="_blank" rel="noreferrer" className="rounded-xl bg-[#25D366] px-4 py-2 text-xs font-black text-white">Recordar pago por WhatsApp</a> : <span className="rounded-lg bg-amber-100 px-3 py-2 text-xs font-black text-amber-900">Falta teléfono para recordar</span>) : null}</div></td>
