...
import EntregaRopaTarjeta from "@/components/EntregaRopaTarjeta";
...
function SelectorTalla({ etiqueta, valor, cambiar }: { etiqueta: string; valor: string; cambiar: (valor: string) => void }) { return <label className="text-sm font-bold">{etiqueta}<select value={valor} onChange={(evento) => cambiar(evento.target.value)} className="input-preregistro mt-1"><option value="">Seleccionar</option>{TALLAS_DISPONIBLES.map((talla) => <option key={talla} value={talla}>{talla}</option>)}</select></label>; }
