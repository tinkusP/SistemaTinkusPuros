...
export default function TokenRegistroView() {
  ...
  return <main className="space-y-6">
    ...
    <section className="overflow-x-auto rounded-2xl bg-white p-5"><div className="mb-4 grid gap-3 sm:grid-cols-[1fr_240px]"><input value={buscar} onChange={(evento) => setBuscar(evento.target.value)} placeholder="Buscar token, nombre o CI" className="rounded-xl border p-3"/><select value={filtro} onChange={(evento) => setFiltro(evento.target.value)} className="rounded-xl border p-3"><option value="TODOS">Disponibles y utilizados</option><option value="DISPONIBLE">Solo disponibles</option><option value="UTILIZADO">Solo utilizados</option><option value="VENCIDO">Solo vencidos</option></select></div><table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-[#74122A] text-white"><tr>{["Token", "Cupo", "Estado", "Vence", "Pago límite", "Generado por", "Utilizado por", ...(puedeConfigurar?["Cuota"]:[]), "Acción"].map((titulo) => <th className="p-3" key={titulo}>{titulo}</th>)}</tr></thead><tbody>{tokensVisibles.map((token) => <tr key={token._id} className="border-b"><td className="p-3"><b className="font-mono">{token.codigo}</b><div className="mt-2 flex gap-2"><button type="button" onClick={() => navigator.clipboard.writeText(token.codigo).then(() => toast.success("Token copiado"))} className="rounded bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">Copiar</button><BotonQrToken codigo={token.codigo}/></div></td><td>{token.sexoCupo || "Se define al registrarse"}</td><td><b>{token.estado}</b></td><td>{new Date(token.fechaExpiracion).toLocaleString("es-BO")}<small className="block font-bold text-amber-700">{token.estado === "DISPONIBLE" && restante(token.fechaExpiracion)}</small></td><td>{token.plazoPagoHoras} h después del registro</td><td>{nombre(token.generadoPor)}</td><td>{nombre(token.utilizadoPor)}</td>{puedeConfigurar&&<td>{token.cuotaId ? "Cuota vinculada" : "—"}</td>}<td>{token.estado === "DISPONIBLE" && <button type="button" onClick={() => setTokenAnular(token)} className="text-red-700 underline">Anular</button>}</td></tr>)}</tbody></table></section>
