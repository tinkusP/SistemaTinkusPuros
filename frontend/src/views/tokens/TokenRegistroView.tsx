...
import { anularToken, crearToken, eliminarToken, guardarConfiguracionTokens, obtenerConfiguracionTokens, obtenerTokens, type TokenRegistro } from "@/api/TokenRegistroApi";
...
const fechaHoraBolivia = (fecha?: string) => fecha ? new Date(fecha).toLocaleString("es-BO", { timeZone: "America/La_Paz", dateStyle: "medium", timeStyle: "medium" }) : "—";
...
export default function TokenRegistroView() {
  ...
  const [tokenEliminar, setTokenEliminar] = useState<TokenRegistro | null>(null);
  ...
  const eliminar = useMutation({ mutationFn: eliminarToken, onSuccess: async (respuesta) => { toast.success(respuesta.message); await refrescar(); }, onError: (error: Error) => toast.error(error.message) });
  const cupos = cfg.data?.cupos;
  const gestion = cfg.data?.gestion;
  const puedeConfigurar = cfg.data?.puedeConfigurar === true;
  const tokensVisibles = (q.data || []).filter((token) => (filtro === "TODOS" || token.estado === filtro) && `${token.codigo} ${nombre(token.utilizadoPor)} ${token.utilizadoPor?.ci || ""}`.toLowerCase().includes(buscar.toLowerCase()));
  ...
  return <main className="space-y-6">
    ...
    {!puedeConfigurar && <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><strong>Vista de ayudante.</strong> Solo puedes generar, consultar y anular los tokens creados por ti. La configuración de pagos y los QR bancarios están restringidos.</section>}
    {puedeConfigurar && form && <section className="rounded-2xl border bg-white p-5"><h2 className="text-xl font-black text-[#74122A]">Configuración de gestión</h2><label className="mt-4 flex items-start gap-3 rounded-xl border border-[#C59A3A] bg-amber-50 p-4"><input type="checkbox" checked={form.requerirTokenRegistro} onChange={(evento) => setForm({ ...form, requerirTokenRegistro: evento.target.checked })} className="mt-1 h-5 w-5"/><span><strong className="block text-[#74122A]">Exigir token de invitación para crear cuenta</strong><small className="text-slate-600">Desactívalo para registros presenciales o locales. El usuario abrirá directamente el formulario, pero su cuenta seguirá pendiente de alta administrativa.</small></span></label><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Campo titulo="Cupos hombres" valor={form.cupoMaximoHombres} cambiar={(valor) => setForm({ ...form, cupoMaximoHombres: valor })}/><Campo titulo="Cupos mujeres" valor={form.cupoMaximoMujeres} cambiar={(valor) => setForm({ ...form, cupoMaximoMujeres: valor })}/><Campo titulo="Tarifa internos FCPN (Bs)" valor={form.tarifaInterno} cambiar={(valor) => setForm({ ...form, tarifaInterno: valor })}/><Campo titulo="Tarifa externos (Bs)" valor={form.tarifaExterno} cambiar={(valor) => setForm({ ...form, tarifaExterno: valor })}/><Campo titulo="Vigencia del token (horas)" valor={form.vigenciaTokenHoras} cambiar={(valor) => setForm({ ...form, vigenciaTokenHoras: valor })}/><Campo titulo="Tiempo para pagar primera cuota (horas)" valor={form.plazoPrimeraCuotaHoras} cambiar={(valor) => setForm({ ...form, plazoPrimeraCuotaHoras: valor })}/><Campo titulo="Cantidad de bloques" valor={form.cantidadBloques} cambiar={(valor) => setForm({ ...form, cantidadBloques: valor })}/></div><p className="mt-3 rounded-xl bg-blue-50 p-3 text-sm text-blue-800"><strong>Plazos siguientes:</strong> los planes de 3 cuotas reciben 7 días después de cada aprobación; los planes de 2 cuotas reciben 14 días. La primera cuota de los planes de 3 pagos es fija en Bs 300.</p><button onClick={() => guardar.mutate(form)} disabled={guardar.isPending} className="mt-4 rounded-xl bg-slate-800 px-5 py-3 font-bold text-white disabled:opacity-50">Guardar configuración</button></section>}
    {puedeConfigurar && <ConfiguracionQrPago/>}
    ...
    <section className="overflow-x-auto rounded-2xl bg-white p-5"><div className="mb-4 grid gap-3 sm:grid-cols-[1fr_240px]"><input value={buscar} onChange={(evento) => setBuscar(evento.target.value)} placeholder="Buscar token, nombre o CI" className="rounded-xl border p-3"/><select value={filtro} onChange={(evento) => setFiltro(evento.target.value)} className="rounded-xl border p-3"><option value="TODOS">Disponibles y utilizados</option><option value="DISPONIBLE">Solo disponibles</option><option value="UTILIZADO">Solo utilizados</option><option value="VENCIDO">Solo vencidos</option></select></div><table className="w-full min-w-[1200px] text-left text-sm"><thead className="bg-[#74122A] text-white"><tr>{["Token", "Cupo", "Estado", "Vence", "Pago límite", "Generado por", "Utilizado por", "Fecha y hora de uso", ...(puedeConfigurar?["Cuota"]:[]), "Acción"].map((titulo) => <th className="p-3" key={titulo}>{titulo}</th>)}</tr></thead><tbody>{tokensVisibles.map((token) => <tr key={token._id} className="border-b"><td className="p-3"><b className="font-mono">{token.codigo}</b><div className="mt-2 flex gap-2"><button type="button" onClick={() => navigator.clipboard.writeText(token.codigo).then(() => toast.success("Token copiado"))} className="rounded bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">Copiar</button><BotonQrToken codigo={token.codigo}/></div></td><td>{token.sexoCupo || "Se define al registrarse"}</td><td><b>{token.estado}</b></td><td>{fechaHoraBolivia(token.fechaExpiracion)}<small className="block font-bold text-amber-700">{token.estado === "DISPONIBLE" && restante(token.fechaExpiracion)}</small></td><td>{token.plazoPagoHoras} h después del registro</td><td>{nombre(token.generadoPor)}</td><td>{nombre(token.utilizadoPor)}</td><td className="whitespace-nowrap">{fechaHoraBolivia(token.fechaUtilizado)}</td>{puedeConfigurar&&<td>{token.cuotaId ? "Cuota vinculada" : "—"}</td>}<td><div className="flex flex-col items-start gap-2">{token.estado === "DISPONIBLE" && <button type="button" onClick={() => setTokenAnular(token)} className="font-bold text-amber-700 underline">Anular</button>}{token.estado !== "UTILIZADO" && <button type="button" onClick={() => setTokenEliminar(token)} className="font-bold text-red-700 underline">Eliminar</button>}</div></td></tr>)}</tbody></table></section>
    {tokenAnular && (
      <ModalAnular token={tokenAnular} procesando={anular.isPending} cerrar={() => setTokenAnular(null)} confirmar={() => anular.mutate(tokenAnular._id, { onSuccess: () => setTokenAnular(null) })}/>
    )}
    {tokenEliminar && (
      <ModalEliminar token={tokenEliminar} procesando={eliminar.isPending} cerrar={() => setTokenEliminar(null)} confirmar={() => eliminar.mutate(tokenEliminar._id, { onSuccess: () => setTokenEliminar(null) })}/>
    )}
  </main>;
}
...
function ModalEliminar({ token, procesando, cerrar, confirmar }: { token: TokenRegistro; procesando: boolean; cerrar: () => void; confirmar: () => void }) { return <div className="fixed inset-0 z-[150] grid place-items-center bg-black/65 p-4" role="dialog" aria-modal="true" aria-labelledby="titulo-eliminar-token"><section className="w-full max-w-md rounded-3xl bg-white p-7 text-center shadow-2xl"><div className="text-4xl">🗑️</div><h2 id="titulo-eliminar-token" className="mt-4 text-2xl font-black text-[#74122A]">Eliminar token no utilizado</h2><p className="mt-3">Se borrará definitivamente el token <strong className="font-mono">{token.codigo}</strong>. Esta acción no está disponible para tokens utilizados.</p><div className="mt-6 grid grid-cols-2 gap-3"><button type="button" onClick={cerrar} disabled={procesando} className="rounded-xl border px-4 py-3 font-bold">Cancelar</button><button type="button" onClick={confirmar} disabled={procesando} className="rounded-xl bg-red-700 px-4 py-3 font-bold text-white disabled:opacity-50">{procesando ? "Eliminando..." : "Sí, eliminar"}</button></div></section></div>; }
