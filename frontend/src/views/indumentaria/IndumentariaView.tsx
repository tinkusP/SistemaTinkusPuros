      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left">
        ...
        <tbody>{fraternos.map((fraterno) => {
          ...
          return <tr key={fraterno._id} className="border-b border-slate-100 align-top hover:bg-slate-50/70">
            <td className="p-4"><p className="font-black text-slate-800">{nombreFraterno(fraterno)}</p><span className={`mt-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${fraterno.estado === "ACTIVO" ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}>{fraterno.estado}</span></td>
            ...
            {seccion !== "INDUMENTARIA" && <>
              ...
              <td className="p-4 text-right"><button type="button" disabled={!prendaPrincipal || mutacion.isPending || (!entregada && !tallaCampo(fraterno._id, tipo))} onClick={() => prendaPrincipal && alternarEntrega(fraterno._id, prendaPrincipal, tallaCampo(fraterno._id, tipo))} className={`rounded-xl px-4 py-2 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-40 ${entregada ? "bg-slate-600" : "bg-emerald-600"}`}>{entregada ? "Marcar devuelto" : "Marcar entregado"}</button></td>
            </>}
